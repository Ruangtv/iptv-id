...
class RctiPlus {
  ...
  async getHLS() {
    ...
    try {
      ...
      const fileName = fileNameRegex.exec(url)[0].replace("/", "").replace(".m3u8", "")
      fileNameRegex = new RegExp(fileName, "gi")
      if (url) {
        const hls = await fetcher(url, ops).then((r) => r.text()).then((r) => r.replace(fileNameRegex, `${baseUrl}${fileName}`))
